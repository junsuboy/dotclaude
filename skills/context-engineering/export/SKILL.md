...
6. **로컬 경로 노출 금지 · 첨부 우선.** 외부 문서(JIRA/Confluence)에 작업 폴더의 다른 산출물을 가리켜야 할 때 **로컬 절대경로(`/Users/...`)를 절대 넣지 않는다.**
   - (a) 파일 첨부가 가능하면 **첨부**한다.
   - (b) 첨부 업로드가 불가능하면 — **현재 `mcp__atlassian__*` 에는 첨부 업로드 도구가 없음(2026-06 확인): Jira/Confluence 모두 첨부 API 미노출** — 본문엔 **파일명만** 남기고(예: `90-SUMMARY.md`), 사용자에게 **"이 파일들을 이 티켓/페이지에 직접 첨부해 주세요"** 라고 명시적으로 가이드한다.
